...
saledata = saledata.sort_values(by = ['Region', 'Channel', 'Category','Item_Type', 'year','month', 'Gender'])

years = list(saledata['year'].unique())
years.sort()

# App & Server - Layout

app = dash.Dash(__name__)                       # app 정의
app.title = ('Dashboard | Sales Data')          # title (웹브라우저탭)
server = app.server                             # server 정의

app.layout = html.Div([                          # Layout 정의
    html.H2('Sales Dashboard with Dash',        # Title 정의
            style = {'textAlign' : 'center',          # H2 속성: 가운데정렬
            'marginBotton' : 10, 'marginTop' : 10}),  # H2 속성: 상하단 10
...
def update_output(val):
    ...
    for i in range(len(pies)):
        df_fig = saledata[saledata['year'] == val]
...
def update_output(val):
    ...
    # reg - unique value's
    reg = saledata['Region'].unique()
    
    # data by Region
    figures = []
    
    for i in range(len(reg)):
        df_fig = saledata[(saledata['year'] == val) & (saledata['Region'] == reg[i])]
...
def update_output(val):
    ...
    # Sales by Country
    df_con = saledata[saledata['year'] == val]
...
def update_output(val):
    ...
    for yr in years:
        ...
        df_line = saledata[saledata['year'] == yr]
...
# Radar, by Year & Category
@app.callback(Output('radar', 'figure'), [Input('id_year', 'value')])

def update_output(val):
    
    df_rad = saledata.loc[:,['Category','Revenue','year']].groupby(by = ['year','Category'], as_index=False).sum()
...
def update_output(val):
    ...
    # Code3 by Country
    df_code = saledata.loc[:,['Country','Code3']].drop_duplicates()

    # data
    df_map = saledata[saledata['year'] == val]
...
def update_output(val):
    ...
    df_san = saledata[saledata['year'] == val].iloc[:,[13,4,11,9]]
